...
import api from 'api';
import { IUpdateCafeReviewProps } from 'api/cafeReviews';
...
const CafeReview: React.FC<IProps> = ({ id, review }) => {
  ...
  const mutation = useMutation(
    (data: IUpdateCafeReviewProps) => api.cafeReviews.updateReview(data),
    {
      onSuccess: () => {
        alert('리뷰가 작성되었습니다.');
        setSubmitting(false);
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (review) {
      mutation.mutate({ id, cafeId: review.cafeId, rating, text });
    }
  }
